...
class GeoSort():
    ...
    def read_lines(self):
        ...
        bool_gmsh = [f'BooleanDifference{{ Volume{{{Volumes[0].split("(")[1].split(")")[0]}}}; {self.boolean};}}{{ Volume{{{Volumes[1].split("(")[1].split(")")[0]}}}; {self.boolean};}}']
        return ['SetFactory("OpenCASCADE");\n', cl__s, points, lines, CurveLoops, Surfaces, PlaneSurfaces, SurfaceLoops, Volumes, bool_gmsh]
